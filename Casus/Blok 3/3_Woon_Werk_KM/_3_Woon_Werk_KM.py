...
SQL_command = ("SELECT DATEPART('m', datum), DATEPART('yyyy',datum), km_van_kantoor " +
...
for i in range(0, len(Editable_list)):    
    ...
    for x in range(0, len(Editable_list)):        
        if Date[0] == Editable_list[x][0] and Date[1] == Editable_list[x][1]:            
            Temp += Editable_list[x][2] * 2    
    Temp_list
...
print('{:<2}{:<12}{:<0}'.format('','Datum', 'Totaal woon werk verkeer'))

for i in range(0, len(Day_home_list)):
    print('{:<2}{:<0}-{:<10}{:<0}'.format('', Day_home_list[i][0] ,Day_home_list[i][1], Day_home_list[i][2]))